...
class Keithley2400(RexSupport):
    def __init__(self, config, name="Keithley2400", connect_to_rex=True):
        ...
        self.measurements = {
            "voltage (mV)": Measurement(data=[], unit="mV"),
            "current (mA)": Measurement(data=[], unit="mA"),
            "resistance (Ω)": Measurement(data=[], unit="ohm"),
            "timestamp (s)": Measurement(data=[], unit="s"),
            "status": Measurement(data=[], unit="dimensionless"),
        }
    ...
    def configure_device(self):
        ...
        self.instrument.write(
            f":SOUR:FUNC {self.measurement_settings['source_mode']}"
        )  # Current or voltage is sourced to sample
        self.instrument.write(
            f":SOUR:{self.measurement_settings['source_mode']}:MODE FIX"
        )  # Fixed sourcing mode

        if self.measurement_settings["source_mode"] == "CURR":
            self.instrument.write(
                f":SOUR:{self.measurement_settings['source_mode']}:RANG {self.measurement_settings['current_range']}"
            )  # Current source range
            self.instrument.write(
                f":SOUR:{self.measurement_settings['source_mode']}:LEV {self.measurement_settings['current_level']}"
            )  # Current source amplitude
        elif self.measurement_settings["source_mode"] == "VOLT":
            self.instrument.write(
                f":SOUR:{self.measurement_settings['source_mode']}:RANG {self.measurement_settings['voltage_range']}"
            )  # Voltage source range
            self.instrument.write(
                f":SOUR:{self.measurement_settings['source_mode']}:LEV {self.measurement_settings['voltage_level']}"
            )  # Voltage source amplitude

        self.instrument.write(
            f":SENS:FUNC '{self.measurement_settings['sense_mode']}'"
        )  # Measure voltage or current

        if self.measurement_settings["sense_mode"] == "VOLT":
            self.instrument.write(
                f":SENS:{self.measurement_settings['sense_mode']}:PROT {self.measurement_settings['compliance_voltage']}"
            )  # Compliance voltage
            self.instrument.write(
                f":SENS:{self.measurement_settings['sense_mode']}:RANG {self.measurement_settings['measurevolt_range']}"
            )  # Measure current range
        elif self.measurement_settings["sense_mode"] == "CURR":
            self.instrument.write(
                f":SENS:{self.measurement_settings['sense_mode']}:PROT {self.measurement_settings['compliance_current']}"
            )  # Compliance current
            self.instrument.write(
                f":SENS:{self.measurement_settings['sense_mode']}:RANG {self.measurement_settings['measurecurrent_range']}"
    ...
    def measure(self):
        ...
        self.measurements["voltage (mV)"] = Measurement(
            data=[V],
            unit="mV",
        )
        ...
        self.measurements["resistance (Ω)"] = Measurement(
            data=[R],
            unit="ohm",
        )

        self.measurements["timestamp (s)"] = Measurement(
            data=[T],
            unit="s",
        )

        self.measurements["status"] = Measurement(
            data=[S],
            unit="dimensionless",
        )
