...
class Gl100(RexSupport):
    ...
    __toml_config__ = {
        "device.GL100_Dye_Laser": {
            ...
            "end_position": {"_value": 600, "_description": "End wavelength in (nm)"},
            "dye": {"_value": "C540", "_description": "Dye used for the experiment"},
        }
    }
    ...
    def __init__(self, config: str, name: str = "GL100_Dye_Laser", connect_to_rex=True):
        ...
        self.scan_data = self._generatescan_data(
            self.start_measurement_position, self.end_measurement_position
        )
    ...
    def check_step(self, step_size):
        ...
        if self.actual_step_size != step_size:
            self.logger.warning(
                f"Warning: Desired step size {step_size}nm is smaller than minimum step size {self.min_step_size}nm"
            )
            self.logger.warning(
                f"Using minimum step size of {self.min_step_size}nm instead"
            )
    ...
    def _generatescan_data(self, start, stop):
        ...
        absolute_steps_from_calibrated = round(
            absolute_pos_from_calibrated * self.steps_per_nm
        )
        ...
        for point_idx in range(num_desired_points):
            ...
            absolute_desired_steps = round(
                (desired_wavelength - self.initial_position) * self.steps_per_nm
            )
    ...
    def measure(self):
        ...
        self.measurements["wavelength (nm)"] = Measurement(
            data=[current_position["actual_wavelength"]],
            unit="nm",
        )
        self.measurements["desired wavelength (nm)"] = Measurement(
            data=[current_position["desired_wavelength"]],
            unit="nm",
        )
        self.measurements["wavelength error (nm)"] = Measurement(
            data=[current_position["wavelength_error"]],
            unit="nm",
        )
        if self.connect_to_rex:
        ...
        for port in ports:
            try:
                with serial.Serial(port.device, baudrate, timeout=timeout) as ser:
                    ...
                    cleaned_responses = [
                        line.decode().strip() for line in responses if line.strip()
                    ]
    ...
    def move_to_start(self):
        ...
        if current_pos <= target_pos:
            ...
            self.logger.warning(
                "Moving backwards to start a scan induces a potential backlash error!"
            )
