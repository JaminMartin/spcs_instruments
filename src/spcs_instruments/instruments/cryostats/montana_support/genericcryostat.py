...
class PidScheduleItem:
    ...
    def __lt__(self, other):
        ...
        elif self.kc != other.kc:
            return self.kc < other.kc
        elif self.ti != other.ti:
...
class GenericCryostat(instrument.Instrument):
    ...
    def __init__(self, ip, port, version="v1", verbose=False, tunnel=False):
        super().__init__(
            ip=ip, port=port, version=version, verbose=verbose, tunnel=tunnel
        )
        return
    ...
    def cryo_thermometer_channels(self) -> [tuple]:
        ...
        return list(
            set(
                [
                    (x[4], x[6])
                    for x in [x.split("/") for x in self.get("/")]
                    if is_tc(x)
                ]
            )
        )
    ...
    def _tc_root(self, channel):
        return self.cryo_tc_channel_endpoint_root(channel)
    ...
    def enable_controller(self, channel, mode, temperature):
        ...
        return self.call_method(
            root + "/methods/enableController(ControlMode:mode,double:temperature)",
            {"mode": mode, "temperature": temperature},
        )

    def disable_controller(self, channel):
        return self.call_method(
            self.cryo_tc_channel_endpoint_root(channel) + "/methods/disableController()"
        )
    ...
    def set_tc_pid_schedule(
        self, channel: (str, str), sched: [PidScheduleItem]
    ) -> None:
        """Store the PID schedule on the system."""
        data = {
            "rows": [
                {"temperature": r.temperature, "kc": r.kc, "ti": r.ti, "td": r.td}
                for r in sched
            ]
        }
...
def register_cryo_class(cls, create=None):
    ...
    """
    if create is None:
        _cryo_classes[cls] = lambda ip, tunnel: cls(ip, "v1", False, tunnel)
    else:
        _cryo_classes[cls] = create
    return
...
def register(cls):
    ...
    """
    assert issubclass(cls, GenericCryostat), (
        "only cryostat objects should be registered as cryostats"
    )
...
def connect_cryostat(host: str, tunnel: bool) -> GenericCryostat:
    ...
    try:
        if cryo is None:
            for p in [
                instrument.Rest_Ports.scryostation_hlm,
                instrument.Rest_Ports.xpcryostation_hlm,
            ]:
                cryo = GenericCryostat(host, port=p, version="v1", tunnel=tunnel)
                if cryo.is_up():
                    break
                cryo.close()
                cryo = None
