...
def tunnel(remote_host, remote_ports, remote_user="tunnel", local_ports=None):
    ...
    if not isinstance(remote_ports, (list, tuple)):
        remote_ports = (remote_ports,)
    if isinstance(remote_ports, (list,)):
        remote_ports = tuple(remote_ports)

    if local_ports is None:
        local_ports = (0,) * len(remote_ports)
    if not isinstance(local_ports, (list, tuple)):
        local_ports = (local_ports,)
    if isinstance(local_ports, (list,)):
        local_ports = tuple(local_ports)

    assert len(local_ports) == len(remote_ports), (
        "Must be a one to one mapping from local to remote"
    )

    remote_binds = [("127.0.0.1", to_port_number(p)) for p in remote_ports]
    local_binds = [("127.0.0.1", to_port_number(p)) for p in local_ports]

    pkey = os.path.join(os.path.join(os.path.expanduser("~"), ".ssh"), "id_rsa")
    ...
    try:
        ...
        raise TunnelError(
            "Could not start tunnel.  Have you copied your ssh keys?"
        ) from None
    return server
...
def _main():
    ...
    with catch_sigint() as got_sigint:
        ...
        local = None
        server = tunnel(
            "192.168.45.115", ports, remote_user="tunnel", local_ports=local
        )
        print("started")
        print(
            f"remote->local ports are {[(ports[i], server.local_bind_ports[i]) for i in range(len(ports))]}"
        )
        while running:
            try:
                print("sleeping")
