...
sys.path.append(
    op.abspath(
        op.join(
            op.dirname(__file__), os.pardir, os.pardir, "commonScripting", "pythonlibs"
        )
    )
)
...
class Rest_Ports(IntEnum):
    ...
    piezo_stepper1_llm = (
        47121  # /etc/services entry is "piezostep1-llm        47121/tcp"
    )
    piezo_stepper2_llm = (
        47122  # /etc/services entry is "piezostep2-llm        47122/tcp"
    )
    piezo_stepper3_llm = (
        47123  # /etc/services entry is "piezostep3-llm        47123/tcp"
    )
    piezo_stepper4_llm = (
        47124  # /etc/services entry is "piezostep4-llm        47124/tcp"
    )
    piezo_stepper5_llm = (
        47125  # /etc/services entry is "piezostep5-llm        47125/tcp"
    )

    pfeiffertc110_llm = (
        47130  # /etc/services entry is "pfeiffertc110-llm     47130/tcp"
    )

    cryostatancillary_llm = (
        47135  # /etc/services entry is "cryostatancillary-llm 47135/tcp"
    )
    ...
    keyencecl3000_llm = (
        47160  # /etc/services entry is "keyencecl3000-llm     47160/tcp"
    )
    keyencesift1000_llm = (
        47161  # /etc/services entry is "keyencesift1000-llm   47161/tcp"
    )
...
def _verifySuccess(op, path, resp) -> None:
    ...
    if resp.status_code not in _success:
        ...
        details = None
        try:
            if "title" in resp.json().keys():
                brief = resp.json()["title"]
...
class Instrument:
    def __init__(self, ip, port, version, verbose=False, tunnel=False):
        ...
        """
        assert not isinstance(port, (list, tuple)), (
            "This class can only talk to a single port"
        )
        ...
        if tunnel:
            try:
                ...
                raise TunnelError(
                    "Could not start the ssh tunnel.  Have you copied your ssh keys?"
                )
...
class ApiError(Exception):
    ...
    def __str__(self):
        ...
        if self.brief:
            result = f"ApiError: {self.op}: {self.response_code}: {self.brief}"
        return result
