...
class TooManyHosts(Exception):
    ...
    def __str__(self):
        return "You may only specify one host."
...
class TooManyUsers(Exception):
    ...
    def __str__(self):
        return "You may only specify one user account."
...
class UnspecifiedHost(Exception):
    ...
    def __str__(self):
        return "You must a host for the copy."
...
def all_tunnels(mirs_server=_mirs_host, rest_port=8080):
    ...
    """
    result = json.loads(
        requests.get(f"http://{mirs_server}:{rest_port}/tunnels").content.decode(
            "utf-8"
        )
    )
    return result
...
def all_ips(mirs_server=_mirs_host, rest_port=8080):
    ...
    """
    result = json.loads(
        requests.get(f"http://{mirs_server}:{rest_port}/ips").content.decode("utf-8")
    )
    return result
...
def lookup_port(miec, miec_port, mirs_server=_mirs_host, rest_port=8080):
    ...
    """
    if isinstance(miec_port, enum.Enum):
        miec_port = miec_port.value
    tunnels = all_tunnels()
    try:
        if miec not in tunnels:
            for k in tunnels.keys():
                miec = k if k.lower() == miec.lower() else miec
...
def rewrite_ssh_args(argv, mirs_server=_mirs_host):
    ...
    if cmd == "mirs-scp":
        ...
        result = ["scp", "-oStrictHostKeyChecking=no"]
    else:
        flags = [
            "-4",
            "-6",
        ...
        result = ["ssh", "-oStrictHostKeyChecking=no"]
    ...
    while i < len(argv):
        if argv[i] in flags or argv[i] == "--help":
            ...
            if argv[i] in ("-p", "-P"):
                sys.stderr.write(
                    "Error: Specifying an alternate port is not supported\n"
                )
                raise PrintUsage()
            ...
            if result[0] == "ssh":
                ...
                def _find_scp_user(fnames):
                    users = [
                        u
                        for u in list({_split_scp_fname(f)[0]: 0 for f in fnames})
                        if u
                    ]
                    if len(users) == 0:
                        users = ["mi@"]
