// tests/basic_test.rs
use spcs_rust_utils::data_handler::{create_time_stamp, sanitize_filename, start_experiment, update_experiment_log};
...
fn test_create_time_stamp() {
    ...
    assert!(re.is_match(&timestamp), "Timestamp does not match expected format: {}", timestamp);


    let header_timestamp = create_time_stamp(true);
    let header_re = Regex::new(r"^\d{2}_\d{2}_\d{4}_\d{2}_\d{2}_\d{2}_\d{3}$").unwrap();
    assert!(header_re.is_match(&header_timestamp), "Header timestamp does not match expected format: {}", header_timestamp);
}
...
fn test_sanitize_filename() {
    ...
    assert_eq!(sanitize_filename("file / name / test"), "file___name___test");
}
...
fn test_start_experiment() {
    ...
    assert!(result.is_ok(), "start_experiment function failed: {:?}", result);
    ...
    let updated_config: Config = toml::from_str(&log_content).expect("Failed to parse log file content");
    assert!(updated_config.experiment.start_time.is_some(), "start_time is not set in the log file");
...
fn test_update_experiment_log() {
    ...
    let toml_content = r#"
        ...
        experiment_description = "This is a test."
    "#;
    log_file.write_all(toml_content.as_bytes()).expect("Failed to write to log file");
    ...
    assert!(result.is_ok(), "update_experiment_log function failed: {:?}", result);
