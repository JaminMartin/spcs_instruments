...
The GL100 driver searches serial ports for the PTI GenStepper controller,
configures its speed, then zeros the stepper during construction. Confirm that
the selected dye, calibrated mechanical position, wavelength limits, and scan
range are safe before creating it.

```python
from spcs_instruments import Gl100

laser = Gl100("config.toml")
for _ in range(laser.total_steps):
    reading = laser.measure()
laser.return_to_zero()
```

Each measurement advances one scan step and records actual wavelength, desired
wavelength, and wavelength error. The supported range is 380–750 nm and the
minimum step is 0.02 nm.
